...
class FourchanParser
{
    public function parseImage($html_content)
    {
        ...
        */
        ...
        // preg_match_all("#href=\"\/\/i.4cdn.org\/\S+\.(jpg|png|gif|webp|jpeg|bmp|webm)#", $html_content, $match);
        preg_match_all("#href=\"\/\/(is2.4chan.org|is3.4chan.org|i.4cdn.org)\/\S+\.(jpg|png|gif|webp|jpeg|bmp|webm)#", $html_content, $match);

        foreach ($match[0] as $key => $value) {
            // $value = substr($value, 8);
            // $value = 'https://' . $value;
            $value = substr($value, 17);
            $value = 'https://i.4cdn' . $value;
